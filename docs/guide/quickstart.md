...
If you are running the proxy locally for yourself, ingress auth is disabled by default
(`plugins/manifest.yaml` → `Ingress Auth & Zero-Trust: enabled: false`). Your upstream
API key passes through transparently — no proxy token needed.

```bash
# ANTHROPIC_API_KEY is already in your environment (e.g. from Claude Code)
curl http://localhost:8090/v1/chat/completions \
...
When ingress auth is enabled (`enabled: true`), clients must authenticate with a proxy
token from `LLM_PROXY_API_KEYS`. Read the generated key from `.env`:
...
## Using with Claude Code / Cursor / Continue / OpenWebUI

Point any OpenAI-compatible client to `http://localhost:8090/v1`.

**Personal mode** (ingress auth disabled — default): use your real upstream API key.
```
Base URL: http://localhost:8090/v1
API Key:  <your real ANTHROPIC_API_KEY or OPENAI_API_KEY>
```

For Claude Code specifically, only one env var needs to change:
```bash
ANTHROPIC_BASE_URL=http://localhost:8090/v1
# ANTHROPIC_API_KEY stays as-is — the proxy forwards it transparently.
```

**Team mode** (ingress auth enabled): use a proxy token from `LLM_PROXY_API_KEYS`.
```
Base URL: http://localhost:8090/v1
API Key:  (one of the tokens in LLM_PROXY_API_KEYS)
```
